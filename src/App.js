...
// Verified users list (admin controlled)
...
const AuthScreen = ({ onAuthSuccess }) => {
  ...
  const handleLogin = () => {
    if (!loginData.email || !loginData.password) return showToast('ሁሉንም መስኮች ይሙሉ!');
    showToast('እየገቡ ነው...');
    setTimeout(() => onAuthSuccess({ name: loginData.email.split('@')[0], email: loginData.email }), 1500);
  };
  ...
  const handleGoogle = () => {
    showToast('Google ግባ...');
    setTimeout(() => onAuthSuccess({ name: 'Google ተጠቃሚ', email: 'user@gmail.com' }), 1500);
  };
...
const MainApp = ({ user, onLogout, accounts, onSwitchAccount, onAddAccount }) => {
  ...
  const [verifyStatus, setVerifyStatus] = useState(null); // null | 'verified' | 'pending'
  const [showAccountSwitcher, setShowAccountSwitcher] = useState(false);
  const ADMIN_CODE = 'HENON2024';

  const [posts, setPosts] = useState([
    { id: 1, type: 'text', author: 'ደ/ዘማርያም ቤ/ክ', initials: 'ደዘ', color: '#B8860B', text: 'የትንሳኤ በዓል ዝግጅት በደመቀ ሁኔታ እየተከናወነ ይገኛል። ሁላችሁም ተጋብዛችኋል!', time: '2 ሰዓት በፊት', prayers: 120, likes: 450, views: '1.2k', comments: [] },
  ...
  const handlePost = () => {
    if (newPost.trim()) {
      setPosts([{ id: Date.now(), type: 'text', author: user.name, initials: user.name.slice(0,2), color: '#B8860B', text: newPost, time: 'አሁን', prayers: 0, likes: 0, views: '1', comments: [] }, ...posts]);
      setNewPost('');
    ...
    <div style={{ backgroundColor: '#1A1508', borderRadius: '20px', marginBottom: '14px', border: '1px solid #2a2010', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-around', borderTop: '1px solid #2a2010', padding: '8px 4px' }}>
        {[
          ...
          { Icon: HandHeart, label: p.prayers, action: () => triggerToast('ጸሎት ተመዝግቧል') },
          { Icon: MessageCircle, label: p.comments.length, action: () => triggerToast('አስተያየቶች') },
          { Icon: Share2, label: 'አጋራ', action: () => triggerToast('ተጋርቷል!') },
  ...
  const renderHome = () => (
    <div style={{ paddingBottom: '20px' }}>
      {/* Stories - horizontal scroll only, no vertical scroll */}
      ...
      <div style={{ backgroundColor: '#1A1508', borderRadius: '16px', padding: '14px', marginBottom: '16px', border: '1px solid #2a2010' }}>
        <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
          ...
          <textarea placeholder="ምስክርነትዎን ወይም ጸሎትዎን ያጋሩ..." value={newPost} onChange={(e) => setNewPost(e.target.value)}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #2a2010', paddingTop: '10px' }}>
          <div style={{ display: 'flex', gap: '4px' }}>
            {[{ Icon: Camera, action: () => setActiveTab('upload') }, { Icon: Clapperboard, action: () => setActiveTab('upload') }, { Icon: Rss, action: () => setActiveTab('live') }].map(({ Icon: Ic, action }, i) => (
              <button key={i} onClick={action} style={{ background: 'none', border: 'none', color: '#B8860B', cursor: 'pointer', padding: '6px', borderRadius: '8px', display: 'flex' }}>
      ...
      {!uploadType ? (
        ...
        <div>
          ...
          <div style={{ background: '#1A1508', borderRadius: '20px', padding: '18px', border: '1px solid #2a2010' }}>
            <div style={{ width: '100%', aspectRatio: uploadType === 'video' ? '16/9' : '1/1', background: '#0D0A06', borderRadius: '14px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginBottom: '14px', cursor: 'pointer', border: '2px dashed #2a2010' }}
              onClick={() => triggerToast('ፋይል ምረጥ...')}>
              <IC size={40} color="#333"><UploadCloud /></IC>
              <div style={{ color: '#B8860B', fontWeight: '700', marginTop: '10px', marginBottom: '4px' }}>ፋይል ምረጥ</div>
              <div style={{ color: '#555', fontSize: '12px' }}>ወይም ይጎትቱ ያስቀምጡ</div>
            </div>
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '75vh' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingBottom: '12px', borderBottom: '1px solid #2a2010', marginBottom: '12px' }}>
            ...
            <IC size={18} color="#555"><SlidersHorizontal /></IC>
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button style={{ background: 'none', border: 'none', color: '#B8860B', cursor: 'pointer', display: 'flex' }}>
              <IC size={20} color="#B8860B"><PlusCircle /></IC>
            </button>
    ...
    return (
      <div style={{ paddingBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button style={{ background: 'none', border: 'none', color: '#B8860B', cursor: 'pointer', display: 'flex' }}>
            <IC size={22} color="#B8860B"><PlusCircle /></IC>
          </button>
        </div>
        {CHATS_DATA.map(c => (
  ...
  // PROFILE — removed saved verses and playlist sections
  const renderProfile = () => (
    <div style={{ paddingBottom: '20px' }}>
  ...
  // SETTINGS with Contact/Support Center
  const renderSettings = () => (
    <div style={{ paddingBottom: '20px' }}>
      ...
      {/* Contact Center */}
      <div style={{ background: 'linear-gradient(135deg,#1A1508,#2a1e08)', borderRadius: '16px', padding: '18px', border: '1px solid #B8860B44', marginTop: '8px', marginBottom: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          {[
            { label: 'ቴክኒካዊ ችግር', color: '#fa709a' },
            { label: 'ሂሳብ', color: '#4facfe' },
            { label: 'ሌላ', color: '#43e97b' },
          ].map(tag => (
            <button key={tag.label} style={{ background: `${tag.color}22`, border: `1px solid ${tag.color}55`, borderRadius: '20px', padding: '4px 12px', color: tag.color, cursor: 'pointer', fontSize: '11px' }}>
              {tag.label}
            </button>
          ))}
        </div>
        <button onClick={() => triggerToast('መልዕክትዎ ተልኳል! በቅርቡ እንደርሳዎታለን።')}
        ...
        <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid #2a2010', display: 'flex', justifyContent: 'space-around' }}>
          {[
            { Icon: Mail, label: 'Email', action: () => triggerToast('support@henon.app') },
            { Icon: Phone, label: 'ስልክ', action: () => triggerToast('+251 911 000 000') },
            { Icon: MessageCircle, label: 'Chat', action: () => { setActiveTab('chat'); } },
          ].map(({ Icon: Ic, label, action }, i) => (
            <button key={i} onClick={action} style={{ background: 'none', border: 'none', color: '#B8860B', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
              <IC size={20} color="#B8860B"><Ic /></IC>
      ...
      <div style={{ background: '#1A1508', padding: '15px 16px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', border: '1px solid #2a2010', marginBottom: '8px' }} onClick={() => triggerToast('ስለ ሄኖን...')}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: '#B8860B22', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <img src={LOGO_SRC} alt="ሄኖን" style={{ width: '26px', height: '26px', objectFit: 'contain', borderRadius: '6px' }} />
          </div>
          <span style={{ fontSize: '14px' }}>ስለ ሄኖን</span>
        </div>
        <IC size={16} color="#444"><ChevronRight /></IC>
      </div>
    </div>
  ...
  const renderAddAccount = () => (
    <div style={{ paddingBottom: '20px' }}>
      ...
      <div style={{ background: '#1A1508', borderRadius: '16px', overflow: 'hidden', border: '1px solid #2a2010', marginBottom: '20px' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #2a2010', fontSize: '11px', color: '#666', textTransform: 'uppercase', letterSpacing: '1px' }}>የሚጠቀሙባቸው ሂሳቦች</div>
        {accounts.map((acc, i) => (
          <div key={i} onClick={() => onSwitchAccount(acc)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderBottom: i < accounts.length - 1 ? '1px solid #2a2010' : 'none', cursor: 'pointer', background: acc.email === user.email ? 'rgba(184,134,11,0.1)' : 'transparent' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: '700', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '5px' }}>{acc.name} {acc.email === user.email && <span style={{ fontSize: '10px', background: '#B8860B22', color: '#B8860B', border: '1px solid #B8860B44', borderRadius: '8px', padding: '1px 6px' }}>አሁን</span>}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <button onClick={() => { triggerToast('አዲስ ሂሳብ ለመፍጠር...'); onAddAccount(); }} style={{ background: 'linear-gradient(90deg,#B8860B,#FFD700)', border: 'none', borderRadius: '14px', padding: '15px', color: '#000', fontWeight: '700', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <UserPlus size={18} color="#000" /> አዲስ ሂሳብ ፍጠር
        </button>
        <button onClick={() => { triggerToast('ሌላ ሂሳብ ለመጠቀም...'); onAddAccount(); }} style={{ background: '#1A1508', border: '1px solid #2a2010', borderRadius: '14px', padding: '15px', color: '#F0E6C8', fontWeight: '600', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <UserCheck size={18} color="#B8860B" /> <span style={{ color: '#F0E6C8' }}>ያለ ሂሳብ ጠቀም</span>
        </button>
        <button onClick={() => { triggerToast('Google ይጠቀሙ...'); }} style={{ background: '#1A1508', border: '1px solid #2a2010', borderRadius: '14px', padding: '15px', color: '#F0E6C8', fontWeight: '600', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <GoogleIcon size={18} /> Google ሂሳብ ጨምር
        </button>
      </div>
    </div>
  );
                ...
                <div style={{ background: 'linear-gradient(135deg,#1A1508,#2a1f08)', padding: '32px 20px 20px', borderBottom: '1px solid #2a2010' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '11px', color: '#B8860B', display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => { setActiveTab('profile'); setMenuOpen(false); }}>
                        ፕሮፋይሌን ይመልከቱ <IC size={10} color="#B8860B"><ChevronRight /></IC>
                      </div>
                    </div>
                  </div>
                </div>
...
const App = () => {
  ...
  const handleAddAccount = () => {
    setUser(null); // Go back to auth to add another account
  };
...
const MyChevronDown = ({ size = 14, color = '#B8860B', strokeWidth = 1.8 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round">
